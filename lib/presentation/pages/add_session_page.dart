      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            /// 🏢 SALLE (CAPACITÉ AUTOMATIQUE)
            BlocBuilder<SalleBloc, SalleState>(
              builder: (context, state) {
                if (state is SalleLoaded) {
                  return DropdownButtonFormField<SalleModel>(
                    decoration: const InputDecoration(labelText: "Salle"),
                    value: selectedSalleModel,
                    items: state.salles
                        .map(
                          (s) => DropdownMenuItem<SalleModel>(
                            value: s,
                            child: Text(
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                final session = SessionModel(
                  ...
                  salle: selectedSalleModel!.name,
                  startTime: startTime,
                  endTime: endTime,
                  price: int.parse(priceController.text),

                  /// 🔥 AUTOMATIQUE DEPUIS LA SALLE
                  totalSeats: selectedSalleModel!.capacity,
                  reservedSeats: widget.session?.reservedSeats ?? 0,
                );
